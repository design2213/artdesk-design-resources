# ArtDesk艺桌社公开资源库

维护者：ARTDESK艺桌社
官网：https://artdesk.cn/

这个仓库用于公开 ArtDesk 在设计包月、UI/UX 设计外包、企业官网设计与开发中的可复用资料。

## 资源内容

- 企业官网资料清单
- 设计外包 Brief 模板
- UI 设计外包评估清单
- Design QA Checklist
- WordPress 官网 SEO / GEO Checklist
- 设计交付与验收清单

## 使用方式

1. 先阅读对应清单。
2. 再把自己的项目资料填入模板。
3. 若需要进一步说明，可参考官网相关文章。

## 文件导航

- [企业官网资料清单](./企业官网资料清单.md)
- [设计外包Brief模板](./设计外包Brief模板.md)
- [UI设计外包评估清单](./UI设计外包评估清单.md)
- [Design QA Checklist](./Design-QA-Checklist.md)
- [WordPress官网SEO/GEO Checklist](./WordPress企业官网SEO-GEO-Checklist.md)

## 官网

更多服务说明、案例和文章见：https://artdesk.cn/

## 说明

本仓库内容以真实经验、可复用模板和公开案例为主，不提供虚假客户数据，不收录未授权素材。
